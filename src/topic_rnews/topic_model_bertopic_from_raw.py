...
if __name__ == "__main__":
    print(datetime.datetime.now(), f': beginning. RAM usage: {round(psutil.virtual_memory().used / 1e9)} GB')
    # get args for import
    args = get_args()

    # import dataframe
    print(datetime.datetime.now(), f': import dataframe. RAM usage: {round(psutil.virtual_memory().used / 1e9)} GB')
    ...
    print(datetime.datetime.now(), f': short lines dropped. generating embeddings. RAM usage: {round(psutil.virtual_memory().used / 1e9)} GB')
    embedding_model = SentenceTransformer("distiluse-base-multilingual-cased-v2")
    embedding_model.encode(news_df["text"].astype(str), show_progress_bar=True)

    print(datetime.datetime.now(), f': define parameters. RAM usage: {round(psutil.virtual_memory().used / 1e9)} GB')
    ...
    print(datetime.datetime.now(), f': beginning topic modeling. RAM usage: {round(psutil.virtual_memory().used / 1e9)} GB')
    topics, probs = topic_model.fit_transform(docs)
    print(datetime.datetime.now(),
          f': finished BERTopic model. Results are: TOPIC LENGTH: {len(topics)}, PROBS LENGTH: {len(probs)}, DOCS LENGTH: {len(docs)}')
    print(datetime.datetime.now(), ': saving model. RAM usage: {round(psutil.virtual_memory().used / 1e9)} GB')
    topic_model.save(args.output_document_path + '.pkl', serialization='pickle')

    print(datetime.datetime.now(), f': exporting to dataframes (one for information on topics found, the other is the updated dataset with topic distribution). RAM usage: {round(psutil.virtual_memory().used / 1e9)} GB')
